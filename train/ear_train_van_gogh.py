...
train_data_path = "../data/train_van_gogh.csv"
path_base = "/path/to/save_path"
save_path = f"{path_base}/van_gogh/ft_model_ear_van_gogh.pt"
...
def load_model():
    global vl_chat_processor, model, vl_gpt
    vl_model_path = model_path
    vl_chat_processor = VLChatProcessor.from_pretrained(vl_model_path)
    vl_gpt = AutoModelForCausalLM.from_pretrained(
        vl_model_path, trust_remote_code=True
    )
...
def train_model(device):
    ...
    load_model()
    global vl_chat_processor, model, vl_gpt
    finetuner = FineTunedModel(model)
    ...
    unsafe_prompts = data["unsafe_prompt"]
    safe_prompts = data["safe_prompt"]
        ...
        for i in tqdms:
            ...
            loss = criteria(negative_hidden_states,
                            target_hidden_states - (
                                    negative_guidance * (positive_hidden_states - neutral_hidden_states)))
            ...
            if num_steps in [286, 576]:
                intermediate_save_path = f"{path_base}/ft_model_ear_van_gogh_d{_ + 1}_i{num_steps}.pt"
            ...
            positive_next_token = torch.cat(
                [positive_next_token.unsqueeze(dim=1), positive_next_token.unsqueeze(dim=1)], dim=1).view(-1)
